...
const DEFAULT_CONFIG: SentryConfig = {
  ...
  release: process.env.MJS_SENTRY_RELEASE || 'superset@4.1.3',
...
export function initializeSentry(config?: Partial<SentryConfig>): void {
  try {
    ...
    const sentryDsn = process.env.MJS_SENTRY_DSN;
    const sentryEnvironment = process.env.MJS_SENTRY_ENVIRONMENT;
    ...
    if (process.env.MJS_SENTRY_RELEASE) {
      finalConfig.release = process.env.MJS_SENTRY_RELEASE;
    }
    Sentry.init({
      dsn: finalConfig.dsn,
      sendDefaultPii: true,
